...
int *h_color;
cudaMallocManaged(&h_color,sizeof(int)*num_of_nodes);
  int start, edgeno;   
...
//  cudaMemcpy(d_color,h_ color, sizeof(int)*num_of_nodes, cudaMemcpyHostToDevice);
//  cudaMemcpy(d_cost, h_cost, sizeof(int)*num_of_nodes, cudaMemcpyHostToDevice);

  //bind the texture memory with global memory
  cudaBindTexture(0,g_graph_node_ref,h_graph_nodes, sizeof(Node)*num_of_nodes);
  cudaBindTexture(0,g_graph_edge_ref,h_graph_edges,sizeof(Edge)*num_of_edges);
  ...
  cudaMemcpy(&h_cost[source],&zero,sizeof(int),cudaMemcpyHostToDevice);
  ...
  int h_overflow = 0;
 // int h_overflow;
//  cudaMallocManaged(&&h_overflow,sizeof(int));
  cudaMemset(&h_overflow,0,sizeof(int));
  int *d_overflow;
    ...
    if(k%2 == 0){
      if(num_of_blocks == 1){
        BFS_in_GPU_kernel<<< grid, threads >>>(d_q1,d_q2, h_graph_nodes, 
            h_graph_edges, h_color, h_cost,num_t , tail,GRAY0,k,d_overflow);
      }
        ...
        BFS_kernel_multi_blk_inGPU
          <<< grid, threads >>>(d_q1,d_q2, h_graph_nodes, 
              h_graph_edges, h_color, h_cost, num_td, tail,GRAY0,k,
      ...
      if(num_of_blocks == 1){
        BFS_in_GPU_kernel<<< grid, threads >>>(d_q2,d_q1, h_graph_nodes, 
            h_graph_edges, h_color, h_cost, num_t, tail,GRAY1,k,d_overflow);
      }
      else if(num_of_blocks <= NUM_SM){
        (cudaMemcpy(num_td,&num_t,sizeof(int),
                    cudaMemcpyHostToDevice));
        BFS_kernel_multi_blk_inGPU
          <<< grid, threads >>>(d_q2,d_q1, h_graph_nodes, 
              h_graph_edges, h_color, h_cost, num_td, tail,GRAY1,k,
              switch_kd, max_nodes_per_block_d, global_kt_d,d_overflow);
        (cudaMemcpy(&switch_k,switch_kd, sizeof(int),
                    cudaMemcpyDeviceToHost));
        if(!switch_k){
