...
//  h_Ax_vector=(float*)malloc(sizeof(float)*dim); 
  cudaMallocManaged(&h_Ax_vector,sizeof(float)*dim);

//  h_x_vector=(float*)malloc(sizeof(float)*dim);
  cudaMallocManaged(&h_x_vector,sizeof(float)*dim);
...
//	cudaMalloc((void **)&d_Ax_vector,dim*sizeof(float));
	cudaMemset( (void *) h_Ax_vector, 0, dim*sizeof(float));
...
//	cudaMemcpy(d_x_vector, h_x_vector, dim*sizeof(int), cudaMemcpyHostToDevice);
  ...
  cudaFuncSetCacheConfig(spmv_jds, cudaFuncCachePreferL1);
	...
	for (int i= 0; i<50; i++)
	spmv_jds<<<grid, block>>>(h_Ax_vector,
  				h_data,h_indices,h_perm,
				h_x_vector,d_nzcnt,dim);
