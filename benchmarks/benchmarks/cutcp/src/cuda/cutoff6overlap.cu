...
cudaMallocManaged(&regionZeroAddr,lnall * sizeof(float));
...
cudaMallocManaged(&binBaseAddr,nbins * BIN_DEPTH * sizeof(float4));
...
//  cudaMalloc((void **) &regionZeroCuda, lnall * sizeof(ener_t));
  CUERR;
  cudaMemset(regionZeroAddr, 0, lnall * sizeof(ener_t));
  CUERR;
  if (verbose) {
...
//      cudaMemcpyHostToDevice);
  CUERR;
  binZeroCuda = binBaseAddr + ((c * binDim.y + c) * binDim.x + c) * BIN_DEPTH;
  ...
  for (zRegionIndex = 0;  zRegionIndex < zRegionDim;  zRegionIndex++) {
    ...
    cuda_cutoff_potential_lattice6overlap<<<gridDim, blockDim, 0>>>(binDim.x, binDim.y,
        binZeroCuda, h, cutoff2, inv_cutoff2, regionZeroAddr, zRegionIndex);
...
//  free(regionZeroAddr);
//  free(binBaseAddr);
cudaFree(regionZeroAddr);
cudaFree(binBaseAddr);
  free(bincntBaseAddr);
  free_atom(extra);
